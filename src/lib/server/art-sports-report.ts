...
export async function generateArtSportsReport(userProfile: UserProfile): Promise<Report> {
  ...
  const rankText = userProfile.rank ? `；你填写的艺体综合分排名/投档排名约第 ${userProfile.rank} 名` : '；未填写艺体排名，将主要按综合分差值匹配';
  const scoreLabel = userProfile.candidateType === 'sports' ? '体育专业投档分/专业分' : '综合分/投档分';
    ...
    strategyInsights: [
      {
        title: '艺体类单独成组判断',
        category: '艺体规则',
        summary: '艺术/体育类投档口径与普通类不同，应重点核对专业类别、综合分、专业成绩要求、招生计划和院校专业组。',
    ...
    riskDiagnosis: [
      {
        ...
        level: 'medium',
        message: '艺体预览为最小闭环版本',
        suggestion: '当前按综合分/投档分差值匹配。正式填报前还需核验当年招生章程、专业成绩合格线、单科限制和专业组要求。',
      },
    ],
    riskWarnings: [
      ...result.warnings,
      '艺体类不按普通类位次模型判断，综合分公式和专业合格要求以江西省当年政策为准。',
...
function toRecommendation(record: ArtSportsCandidate, userProfile: UserProfile, scoreForMatch: number): Recommendation {
  ...
  return {
    ...
    reasons: [
      ...
      record.filingRank ? `该专业组${record.year}年投档最低排名为 ${record.filingRank}。` : '该条数据未提供投档最低排名，优先按综合分差值参考。',
      userProfile.rank && record.filingRank ? `你的艺体排名与该专业组投档最低排名相差约 ${Math.abs(record.filingRank - userProfile.rank)} 名。` : '若补充艺体综合分排名/投档排名，风险判断会更精细。',
    ],
    evidence: [{
      year: record.year,
      lowestScore: Math.round(record.filingScore * 1000) / 1000,
    ...
    riskNotes: '当前投档线只到院校专业组层级；组内具体专业、计划数、学费和录取规则需复核江西省招生计划、高校招生章程和当年综合分公式。',
  };
}
