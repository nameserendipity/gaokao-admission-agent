...
export async function generateArtSportsReport(userProfile: UserProfile): Promise<Report> {
  ...
  if (!scoreForMatch || scoreForMatch <= 0) throw new Error(userProfile.candidateType === 'sports' ? '请填写体育综合分/投档分。' : '请填写艺体综合分/投档分。');
    ...
    suitableMajors: [
      {
        ...
        majors: [category],
        reasons: [
          userProfile.candidateType === 'sports' ? '体育类录取以对应类别、院校专业组和体育综合分/投档分为核心参考，体育专业分用于辅助核验。' : '艺术类录取以对应类别、专业组和综合分/投档分为核心参考。',
    ...
    strategyInsights: [
      {
        title: '艺体类单独成组判断',
        category: '艺体规则',
        summary: userProfile.candidateType === 'sports' ? '体育类投档口径与普通类不同，应重点核对体育综合分/投档分、体育专业分、文化成绩要求、招生计划和院校专业组。' : '艺术类投档口径与普通类不同，应重点核对专业类别、综合分、专业成绩要求、招生计划和院校专业组。',
    ...
    riskDiagnosis: [
      {
        ...
        level: 'medium',
        message: '艺体预览为最小闭环版本',
        suggestion: userProfile.candidateType === 'sports' ? '当前按体育综合分/投档分差值匹配。正式填报前还需核验当年招生章程、体育专业分、文化成绩要求、单科限制和专业组要求。' : '当前按综合分/投档分差值匹配。正式填报前还需核验当年招生章程、专业成绩合格线、单科限制和专业组要求。',
      },
    ],
    riskWarnings: [
      ...result.warnings,
      userProfile.candidateType === 'sports'
        ? '体育类不按普通类位次模型判断，体育综合分/投档分、体育专业分、文化成绩要求和投档排序规则以江西省当年政策为准。'
        : '艺术类不按普通类位次模型判断，综合分公式和专业合格要求以江西省当年政策为准。',
      userProfile.candidateType === 'sports'
        ? '体育类报告优先按你填写的体育综合分/投档分匹配；体育专业分仅用于辅助核验，不等同于投档分。'
...
function toRecommendation(record: ArtSportsCandidate, userProfile: UserProfile, scoreForMatch: number): Recommendation {
  ...
  return {
    ...
    reasons: [
      ...
      userProfile.candidateType === 'sports' ? '当前按江西体育类公开投档表中的体育综合分/投档分和体育投档排名匹配，不使用普通类综合分模型；体育专业分仅作辅助核验。' : '\u5f53\u524d\u6309\u827a\u672f\u7c7b\u7efc\u5408\u5206/\u6295\u6863\u5206\u53e3\u5f84\u5339\u914d\u3002',
