...
export default function ReportPage() {
  ...
  const hasMajorGroupRecommendations = allRecommendations.some(item => item.major.name.includes('院校专业组') || item.admissionRecord.notes?.includes('原始专业组信息'));
  const recommendationNotices = [
    noPreferenceMode ? '你没有限定专业或地域，系统已按分数、位次和选科做广谱匹配；后续建议再结合城市、学费和专业接受度做二次筛选。' : null,
    hasMajorGroupRecommendations ? '部分省份按院校专业组公布投档线。报告中的“院校专业组”代表该组投档结果，不是具体专业推荐；组内具体专业、招生人数、学费、校区、录取规则需以招生计划和高校招生章程复核。' : null,
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 lg:py-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-6">
            ...
            <Card className="rounded-[1.5rem] border-slate-200 shadow-sm print:hidden"><CardContent className="p-6 md:p-7"><SectionTitle title="报告问答" subtitle="继续问哪个更稳、是否值得冲、专业组风险是什么" /><div className="mt-5 max-h-96 space-y-3 overflow-auto rounded-2xl bg-slate-50 p-4">{messages.length === 0 && <p className="text-sm text-slate-500">可以问：哪几个志愿最稳？保底还缺不缺？院校专业组要复核什么？</p>}{messages.map(message => <div key={message.id} className={message.role === 'user' ? 'ml-auto max-w-[85%] rounded-2xl bg-blue-700 p-3 text-white' : 'max-w-[85%] rounded-2xl bg-white p-3 text-slate-700 shadow-sm'}><p className="mb-1 text-xs opacity-70">{message.role === 'user' ? '你' : '报告助手'}</p><p className="whitespace-pre-wrap text-sm leading-6">{sanitizeAgentMessageContent(message.content)}</p></div>)}</div><div className="mt-4"><Textarea value={question} onChange={event => setQuestion(event.target.value)} placeholder="例如：我更想稳妥，应该删掉哪些冲刺？" className="min-h-24 rounded-2xl border-slate-200 bg-white" /><Button onClick={handleAsk} disabled={chatLoading || !question.trim()} className="mt-3 rounded-full bg-blue-700 px-5 text-white hover:bg-blue-800">{chatLoading ? '分析中...' : '提交问题'}</Button></div></CardContent></Card>
          </div>
...
function RecommendationCard({ rec, userScore, rankEstimated }: { rec: Recommendation; userScore: number; rankEstimated: boolean }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 transition-colors hover:border-blue-200 hover:bg-blue-50/30">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {rec.major.name.includes('院校专业组') && <Badge variant="secondary" className="rounded-full text-xs">{'\u9700\u67e5\u7ec4\u5185\u4e13\u4e1a'}</Badge>}
      ...
      {rec.major.name.includes('院校专业组') && <p className="mt-3 rounded-xl bg-blue-50 p-3 text-xs leading-5 text-blue-800">该条为院校专业组投档线，不等同于某一个具体专业。正式填报前请核对招生计划中的组内专业、招生人数、校区、学费、录取规则和高校招生章程。</p>}
