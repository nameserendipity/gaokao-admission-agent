...
const T = {
  ...
  art: '\u827a\u672f\u7c7b\u8003\u751f', artDesc: '\u6c5f\u897f\u827a\u672f\u672c\u79d1\u6279\uff1a\u586b\u5199\u6587\u5316\u5206\u3001\u4e13\u4e1a\u5206\u3001\u7efc\u5408\u5206/\u6295\u6863\u5206\u540e\u76f4\u63a5\u751f\u6210\u827a\u4f53\u62a5\u544a', sports: '\u4f53\u80b2\u7c7b\u8003\u751f', sportsDesc: '\u6c5f\u897f\u4f53\u80b2\u672c\u79d1\u6279\uff1a\u586b\u5199\u6587\u5316\u5206\u3001\u4f53\u80b2\u4e13\u4e1a\u5206\u3001\u4f53\u80b2\u7efc\u5408\u5206/\u6295\u6863\u5206\u540e\u76f4\u63a5\u751f\u6210\u827a\u4f53\u62a5\u544a',
  artNotice: '\u827a\u4f53\u7c7b\u5c06\u6309\u6c5f\u897f\u827a\u672f/\u4f53\u80b2\u672c\u79d1\u6279\u6295\u6863\u7ebf\u72ec\u7acb\u751f\u6210\u62a5\u544a\uff0c\u4e0d\u5957\u7528\u666e\u901a\u7c7b\u4f4d\u6b21\u6a21\u578b\u3002\u5f53\u524d\u4ec5\u652f\u6301\u6c5f\u897f\u3002', province: '\u7701\u4efd', trial: '\u8bd5\u7528', jiangxiOnly: '\u5f53\u524d\u827a\u4f53\u62a5\u544a\u4ec5\u652f\u6301\u6c5f\u897f\u7701\uff0c\u8bf7\u9009\u62e9\u6c5f\u897f\u3002',
  score: '\u6587\u5316\u5206/\u9ad8\u8003\u5206\u6570', scorePh: '\u8bf7\u8f93\u5165\u6587\u5316\u5206', chooseProvince: '\u8bf7\u5148\u9009\u62e9\u7701\u4efd', fullScore: '\u6ee1\u5206\u6309\u0037\u0035\u0030\u5206\u5904\u7406', artCategory: '\u827a\u4f53\u7c7b\u522b', sportsCat: '\u4f53\u80b2\u7c7b', professionalScore: '\u4e13\u4e1a\u5206', compositeScore: '\u7efc\u5408\u5206/\u6295\u6863\u5206', sportsCompositeScore: '\u4f53\u80b2\u7efc\u5408\u5206/\u6295\u6863\u5206',
...
export default function InputPage() {
  ...
  const canProceed = () => {
    if (currentStep === 1 && isArtSports) return formData.province === 'jiangxi' && Number(formData.score) > 0 && Number(formData.professionalScore) > 0 && Number(formData.compositeScore) > 0 && (formData.candidateType === 'sports' || Boolean(formData.artSportsCategory));
      ...
      {currentStep === 1 && <div className="space-y-6"><div><h2 className="mb-2 text-xl font-semibold">{T.basic}</h2><p className="text-sm text-muted-foreground">{T.basicHint}</p></div>
        ...
        {isArtSports && (
          <div className="space-y-4 rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4">
            <div>
              <Label className="text-base font-medium">{T.artCategory}</Label>
              {formData.candidateType === 'art' ? (
                <RadioGroup value={formData.artSportsCategory || undefined} onValueChange={(value: string) => updateFormData({ artSportsCategory: value })} className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-2">
                  {ART_CATEGORIES.map(category => <div key={category} className="flex items-center rounded-lg border bg-white p-2"><RadioGroupItem value={category} id={`art-${category}`} /><Label htmlFor={`art-${category}`} className="ml-2 cursor-pointer text-sm">{category}</Label></div>)}
                </RadioGroup>
              ) : (
                <p className="mt-2 rounded-lg bg-white p-3 text-sm text-slate-700">{T.sportsCat}</p>
              )}
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <Label htmlFor="professionalScore">{formData.candidateType === 'sports' ? '\u4f53\u80b2\u4e13\u4e1a\u5206' : T.professionalScore}</Label>
                <Input id="professionalScore" type="number" step="0.001" value={formData.professionalScore ?? ''} onChange={e => updateFormData({ professionalScore: parseFloat(e.target.value) || undefined })} className="mt-2 bg-white" />
                {formData.candidateType === 'sports' && <p className="mt-2 text-xs leading-5 text-emerald-700">{'\u4f53\u80b2\u4e13\u4e1a\u5206\u7528\u4e8e\u8f85\u52a9\u6838\u9a8c\uff0c\u4e0d\u76f4\u63a5\u7b49\u540c\u4e8e\u6295\u6863\u5206\u3002'}</p>}
              </div>
              <div>
                <Label htmlFor="compositeScore">{formData.candidateType === 'sports' ? T.sportsCompositeScore : T.compositeScore}</Label>
                <Input id="compositeScore" type="number" step="0.001" value={formData.compositeScore ?? ''} onChange={e => updateFormData({ compositeScore: parseFloat(e.target.value) || undefined })} className="mt-2 bg-white" />
                {formData.candidateType === 'sports' && <p className="mt-2 text-xs leading-5 text-emerald-700">{'\u8bf7\u4f18\u5148\u586b\u5199\u7701\u8003\u8bd5\u9662\u4e00\u5206\u4e00\u6bb5\u6216\u6295\u6863\u8868\u4e2d\u7684\u4f53\u80b2\u7efc\u5408\u5206/\u6295\u6863\u5206\uff0c\u7cfb\u7edf\u5c06\u6309\u8be5\u5206\u6570\u5339\u914d\u3002'}</p>}
              </div>
            </div>
            <div>
              <Label htmlFor="artRank">{formData.candidateType === 'sports' ? '\u4f53\u80b2\u6295\u6863\u6392\u540d\uff08\u53ef\u9009\uff09' : T.artRank}</Label>
              <Input id="artRank" type="number" placeholder={formData.candidateType === 'sports' ? '\u5982\u6709\u4f53\u80b2\u7c7b\u6295\u6863\u6392\u540d\u8bf7\u586b\u5199' : T.artRankPh} value={formData.rank ?? ''} onChange={e => updateFormData({ rank: parseInt(e.target.value) || null })} className="mt-2 bg-white" />
            </div>
          </div>
        )}
