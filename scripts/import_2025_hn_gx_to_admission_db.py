...
CSV=Path(r'D:\codex-workspace-cli\eol-gktoudang-downloads\parsed\admission_2025_hot_missing_added.csv')
HN='\u6e56\u5357'; GX='\u5e7f\u897f'
df=pd.read_csv(CSV,dtype=str,encoding='utf-8-sig').fillna('')
wanted=df[(df['province'].isin([HN,GX])) & (df['year'].astype(str)=='2025')].copy()
print('csv rows',len(df),'wanted',len(wanted), 'db', DB)
def to_int(v):
    s=str(v).strip()
...
print('before target',cur.execute('select province, year, count(*) from admission where year=2025 and province in (?,?) group by province, year', (HN,GX)).fetchall())
check_sql = """
...
print('after target',cur.execute('select province, year, count(*) from admission where year=2025 and province in (?,?) group by province, year order by province', (HN,GX)).fetchall())
con.close()
